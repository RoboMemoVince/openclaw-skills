...
## 4. Multi-Buffering (Ping-Pong Pipeline)

### al.multibuffer (recommended)

Direct API for enabling double-buffering on a tensor. Simplifies the older `compile_hint` approach.

```python
from triton.language.extra.cann import extension as al

al.multibuffer(tensor, size)
```

| Parameter | Type | Description |
|-----------|------|-------------|
| `tensor` | tensor | The tensor to enable multi-buffering on |
| `size` | int | Number of buffer copies (**currently only `2` is supported**) |

**Example:**
```python
@triton.jit
def kernel(a_ptr, b_ptr, BLOCK: tl.constexpr):
    offs = tl.arange(0, BLOCK)
    for i in range(0, N, BLOCK):
        a = tl.load(a_ptr + offs + i)
        al.multibuffer(a, 2)  # Enable ping-pong for this load
        result = a * 2.0
        tl.store(b_ptr + offs + i, result)
```

### Via autotune Config

Multi-buffering can also be toggled globally per autotune config:

```python
@triton.autotune(
    configs=[
        triton.Config({"BLOCK": 1024}, multibuffer=True),
        triton.Config({"BLOCK": 1024}, multibuffer=False),
    ],
    key=["n_elements"],
)
@triton.jit
...
### al.compile_hint (low-level)

Lower-level interface for compiler hints, including multi-buffering and overflow control.
...
|--------|-------|----------|
| `al.multibuffer(tensor, 2)` | Per-tensor, in-kernel | Fine-grained control over specific loads |
| `triton.Config(multibuffer=True)` | Global, compiler-level | Autotune search across buffer strategies |
| `al.compile_hint(t, "multi_buffer", 2)` | Per-tensor, in-kernel | Same as `al.multibuffer` but also supports other hints |
...
| **Buffer** | `al.multibuffer` | Per-tensor ping-pong pipeline |
| | `al.compile_hint` | Compiler optimization hints |
...
## 9. FP8 Scaled Matrix Multiply

### tl.dot_scaled

Matrix multiply with microscaling (MX) format, designed for FP8 low-precision inference.

```python
result = tl.dot_scaled(
    lhs, lhs_scale, lhs_format,
    rhs, rhs_scale, rhs_format,
    acc=None, out_dtype=tl.float32,
)
```

| Parameter | Type | Description |
|-----------|------|-------------|
| `lhs` | tensor | Left matrix in FP8 format (packed as int32 or int8) |
| `lhs_scale` | tensor | Per-block scale factor for lhs (ue8m0, as int8) |
| `lhs_format` | str | Format: `"e4m3"`, `"e5m2"`, `"e2m3"`, `"e3m2"`, `"e2m1"` |
| `rhs` | tensor | Right matrix in FP8 format |
| `rhs_scale` | tensor | Per-block scale factor for rhs |
| `rhs_format` | str | Format (same options as lhs_format) |
| `acc` | tensor/None | Optional accumulator to add result to |
| `out_dtype` | dtype | Output dtype (default: `tl.float32`) |

**Note**: FP8 tensors cannot be stored directly via `tl.store()`. Always convert back to FP16/FP32 before writing to GM. See [pitfalls #8](./triton-ascend-pitfalls.md#8-fp8-type-support-limitations).
