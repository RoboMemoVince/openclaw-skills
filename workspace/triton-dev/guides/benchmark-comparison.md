...
triton-ascend 在 `triton.backends.ascend.testing` 模块中提供了官方的 `do_bench_npu()` 函数：
...
results = do_bench_npu(
    [lambda: kernel(x, y, out, n)],  # 函数列表
    warmup=5,                         # warmup 次数
    active=30,                        # 实际测量次数
    clear_l2_cache=False,             # 是否清 L2 cache
    prof_dir=None,                    # profiler 输出目录（None=临时目录）
    keep_res=False,                   # 是否保留 profiler 结果
)
```

**与 skill 工具的关系：**
- `do_bench_npu()` 是仓库内置的 profiler-based benchmark，底层通过 `torch_npu` profiler 采集 kernel 时间
- skill 的 `tools/utils.py` 中 `profiler_wrapper()` 和 `print_profiler_kernel_avg_duration()` 提供更细粒度的控制（step-based 去重、per-kernel 拆分、带宽 sanity check）
- **快速 benchmark 用 `do_bench_npu()`，详细 kernel 拆分分析用 `profiler_wrapper()`**

也可以通过环境变量全局切换：
```bash
# 让 triton.testing.do_bench 自动使用 NPU 版本
TRITON_BENCH_METHOD=npu python3 your_script.py
```
