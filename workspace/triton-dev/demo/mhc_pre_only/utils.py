...
NOTE: This is a copy of triton-dev/tools/utils.py for demo self-containment.
      Canonical source: /root/.claude/skills/triton-dev/tools/utils.py
...
def print_profiler_kernel_avg_duration(result_path="./result_profiling"):
    ...
    Computes end-to-end time per step by accounting for kernels that appear
    multiple times per step (e.g. TransposeAiCore appears 3x per step).
    ...
    num_steps = len(step_ids) if step_ids else 1

    # Compute per-step occurrence count: how many times each kernel fires per step
    # e.g. TransposeAiCore fires 3x per step, scgf_fwd_kernel fires 1x
    per_step_count = {}
    for name in durations_us:
        per_step_count[name] = round(kernel_counts[name] / num_steps)

    rows = [
        (name, total / kernel_counts[name], per_step_count[name])
        for name, total in durations_us.items()
    ]
    # Sort by per-step contribution (avg * count) descending
    rows.sort(key=lambda x: x[1] * x[2], reverse=True)

    console = Console()
    table = Table(
        title=f"\nProfiler Kernel Avg Duration (us) | steps={num_steps}\n{latest_csv_path}",
        box=box.SIMPLE_HEAVY,
    )
    table.add_column("Kernel", justify="left", style="cyan", no_wrap=True)
    table.add_column("Avg (us)", justify="right", style="magenta")
    table.add_column("Count/Step", justify="right", style="green")
    table.add_column("Subtotal/Step (us)", justify="right", style="yellow")

    e2e_per_step_us = 0.0
    for name, avg_us, count in rows:
        subtotal = avg_us * count
        e2e_per_step_us += subtotal
        table.add_row(name, f"{avg_us:.3f}", str(count), f"{subtotal:.3f}")
    console.print(table)
    console.print(f"End-to-End Avg per Step (us): {e2e_per_step_us:.3f}")

    return {name: avg_us for name, avg_us, _ in rows}
